...
*/
let kIssuer: String = "https://accounts.google.com";//https://accounts.google.com
...
*/
let kClientID: String? = "780002601256-cutm2pjlacfkohi57uer6j73kkinbvbh.apps.googleusercontent.com";
...
/**
 ...
 For client configuration instructions, see the [README](https://github.com/openid/AppAuth-iOS/blob/master/Examples/Example-iOS_Swift-Carthage/README.md).
*/
let kRedirectURI: String = "com.googleusercontent.apps.780002601256-cutm2pjlacfkohi57uer6j73kkinbvbh:/google";
...
extension AppAuthExampleViewController {
    ...
    @IBAction func v3TranslateBtn(sender: UIButton) {
        v3Translate()
    }
    func v3Translate() {
        var tText = logTextView.text ?? ""
        let param: [String : Any] = [
            "contents": logTextView.text ?? "Default",
            "targetLanguageCode" : "en",
            "mimeType" : "text/plain"
        ]
        let header_oauth2: HTTPHeaders = [
            "Content-Type" : "application/x-www-form-urlencoded; charset=utf-8",
            "Authorization" : "Bearer \(authState!.lastTokenResponse!.accessToken ?? "Default_Token")"
        ]
        
        AF.request("https://translate.googleapis.com/v3/projects/780002601256:translateText", method: .post, parameters: param, headers: header_oauth2).responseJSON { response in
            switch response.result {
            case .success(let data):
                let jsondata = JSON(data)
//                print(jsondata["translations"].arrayValue.first?["translatedText"] ?? "ㅇㄷㄹ며ㅣㅅ")
                self.logTextView.text = "Google Translation v3 \n\n\(jsondata["translations"].arrayValue.first?["translatedText"].stringValue ?? "default")"
            case .failure(let error):
                print(error)
            }
        }
        
        Papago.shared.translator(text: logTextView.text) { text in
...
extension AppAuthExampleViewController {
    ...
    func doAuthWithoutCodeExchange(configuration: OIDServiceConfiguration, clientID: String, clientSecret: String?) {
        ...
        let request = OIDAuthorizationRequest(configuration: configuration,
                                              clientId: clientID,
                                              clientSecret: clientSecret,
                                              scopes: [OIDScopeOpenID, OIDScopeProfile, "https://www.googleapis.com/auth/cloud-platform", "https://www.googleapis.com/auth/cloud-translation"],
                                              redirectURL: redirectURI,
                                              responseType: OIDResponseTypeCode,
                                              additionalParameters: nil)
        ...
        appDelegate.currentAuthorizationFlow = OIDAuthorizationService.present(request, presenting: self) { (response, error) in
            ...
            if let response = response {
                ...
                self.logMessage("Authorization \(response.state ?? "DEFAULT_STATE") response with code: \(response.authorizationCode ?? "DEFAULT_CODE")")
